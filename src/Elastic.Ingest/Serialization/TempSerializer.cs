...
{
	// TODO this will ship inside Elasticsearch.Net at some point

	public class SystemTextJsonSerializer : IElasticsearchSerializer
	{
		private readonly JsonSerializerOptions _indented = new JsonSerializerOptions
		{
			WriteIndented = true
		};
		private readonly JsonSerializerOptions _none = new JsonSerializerOptions
		{
			WriteIndented = false
		};
		...
		{
			deserialize = default!;
			return stream == Stream.Null || (stream.CanSeek && stream.Length == 0);
		}
