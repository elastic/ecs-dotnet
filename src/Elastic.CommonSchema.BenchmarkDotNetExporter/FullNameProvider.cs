	...
	internal static class FullNameProvider
	{
		...
		private static string GetBenchmarkParameters(MethodInfo method, ParameterInstances benchmarkParameters)
		{
			...
			for (var i = 0; i < methodArguments.Length; i++)
			{
				...
				parametersBuilder.Append(GetArgument(benchmarkParameters.GetArgument(methodArguments[i].Name).Value,
					methodArguments[i].ParameterType));
			}

			for (var i = 0; i < benchmarkParams.Length; i++)
			{
